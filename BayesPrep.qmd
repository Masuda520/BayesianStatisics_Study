...
※これ以降この文書では、$y$はデータのことを指し、$\theta$はパラメータのことを指しています。  

### ベイズ統計学におけるパラメータ  
入門・基礎レベルで習う「通常の統計学」――これは「頻度論」と呼ばれます――では、パラメータは未知の**定数**だと考えます。  
例えば、ある小学校の6年生男子全体に対して、その母平均$\theta$を推定する問題を考えてみます。$\theta$は分析者が知ることができない何かしらの定数であって、これを推定するために手元のデータから**推定量**$\hat{\theta}$という値を構成・計算するのでした。（例えば標本平均は母平均を知るための推定量です）  
...
**ベイズ統計学とは、パラメータを確率変数として捉えた統計学のこと**です。
...
【尤度の例】--------------------  
例えば、データ$y_1,y_2,\cdots,y_n$が確率分布$N(\theta, 1)$に独立同一に従っているとき、このことを以下のように表現しました。  
$$
...
それぞれのパラメータの周辺分布は、例えば以下のようにモデリングしてみます。  
$$
\begin{equation*}
\begin{split}
...
事後分布のイメージを図で掴んでみましょう。  
【例】-------------------  
データ$y = (y_1,y_2,\cdots,y_n)$があるとして、$y_i$の平均$\theta$を推定するという、シンプルな問題を考えてみましょう。  
今回は、以下のようなモデリングをしてみます。  
$$
\begin{equation*}
\begin{split}
y_i |\theta &\overset{i.i.d}{\sim} N(\theta, 1) \\
\theta &\sim N(0,\tau ^2)
\end{split}
\end{equation*}
$$

尤度$p(y|\theta) = \prod _{i=1}^n p(y_i | \theta)$が正規分布の密度関数の積であり、事前分布$p(\theta)$が$N(0,\tau^2)$であるようなモデルになっています。$\tau^2$はハイパーパラメータです。  
試しにこのモデルから乱数を作ってみます。  
$\tau ^2 = 10$と設定した場合は次のようになります。  
```{r}
set.seed(10)
tau_squared = 10
theta_rand = rnorm(n = 500 , mean = 0 , sd = sqrt(tau_squared))
y_rand = rnorm(n = 500 , mean = theta_rand , sd = 1)
data = tibble(y = y_rand , theta = theta_rand)
data %>%
  ggplot(aes(x = theta ,  y = y) ) + 
  ylim(-13,16.5) + 
  geom_point() +
  geom_line(tibble(x_vec = seq(-13,13,0.01) , y_vec = 10+50*dnorm(seq(-13,13,0.01),sd=sqrt(tau_squared)) ) , mapping = aes(x = x_vec, y = y_vec), linewidth = 1 , col = "red" ) + 
  labs(title = "散布図はモデルから生成") + 
  theme_minimal()
```

赤線が$\theta$の周辺分布（＝事前分布）を示しています。  
さて、手元のデータ$y\ \ (n=30)$をこの散布図の中に青線で書き加えてみたいと思います。  
```{r}
set.seed(10)
y_obs = rnorm(n = 30 , mean = -3 , sd = 1)
ggplot(tibble(obs = y_obs) , mapping = aes(x = obs)) + 
  geom_histogram(aes(y = after_stat(density)), fill = "blue",binwidth = 0.35) + 
  labs(title = "観測データのヒストグラム (n = 30)：標本平均は-3.344676", x = "y") + 
  theme_minimal()
...
data %>%
  ...
  geom_point() +
  ylim(-13,16.5) + 
  geom_line(tibble(x_vec = seq(-13,13,0.01) , y_vec = 10+50*dnorm(seq(-13,13,0.01),sd=sqrt(tau_squared)) ) , mapping = aes(x = x_vec, y = y_vec), size = 1 , col = "red" ) + 
  geom_hline(yintercept = y_obs , col = "blue" , alpha = 0.2) + 
  labs(title = "散布図はモデルから生成。青線は実データから書き込み") +
  theme_minimal()
```

ここで、青線とかぶってる位置にある乱数を見てみましょう。  
```{r}
data$label = abs(y_rand - mean(y_obs)) < 2
data %>%
  ggplot(aes(x = theta ,  y = y, col = label) ) + 
  geom_point() +
...
**イメージとしては、この部分の$\theta$が事後分布になっています**。  
事後分布を図に書き加えてみます。  
```{r}
posterior_mean = sum(y_obs)/(30+1/tau_squared)
posterior_var = tau_squared/(30*tau_squared + 1)
data %>%
  ggplot(aes(x = theta ,  y = y, col = label) ) + 
  geom_point() +
  scale_color_manual(values = c("FALSE" = "black", "TRUE"="blue"))+  
  ...
  geom_line(tibble(x_vec = seq(-13,13,0.01) , y_vec = 10+50*dnorm(seq(-13,13,0.01),sd=sqrt(tau_squared)) ) , mapping = aes(x = x_vec, y = y_vec), size = 1 , col = "red" ) + 
  geom_hline(yintercept = y_obs , col = "blue" , alpha = 0.2) +  
  geom_line(tibble(x_vec = seq(-13,13,0.01) , 
    y_vec = mean(y_obs)+5*dnorm(seq(-13,13,0.01),mean=posterior_mean,sd=sqrt(posterior_var)) ) ,
      mapping = aes(x = x_vec, y = y_vec), linewidth = 1 , col = "gold" ) + 
  labs(title = "散布図はモデルから生成。青線は実データから書き込み") +
  theme_minimal() + 
  theme(legend.position = "none")
```

事前分布の段階では$\theta$の期待値は0でしたが、事後分布では$\theta$の期待値が-3程度になっていることがわかります。  

さて、この例では「事前分布$p(\theta)$」 「同時分布$p(y,\theta)$」 「事後分布$p(\theta | y)$」を図示してみました。  
あくまでイメージを掴むためのものであり、正確な説明ではないことに注意してください。  
【例　終わり】-------------------  
